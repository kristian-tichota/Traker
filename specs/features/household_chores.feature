...
Feature: Household chores
  Recurring household work, listed on a break and ticked off from there.

  A chore repeats on a cadence: one known occurrence, its anchor, and the days
  between occurrences. The cadence never drifts. A late completion does not buy
  a fresh full period, and an early completion does not clear the occasion.

  A chore belongs to the household rather than to a member: both see one board
  and either may clear it.
